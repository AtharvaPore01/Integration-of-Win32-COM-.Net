...
{
	//Physics
	TCHAR WeightOfPerson[255];
	TCHAR MassOfPerson[255];
	TCHAR GravityOfPlanet[255];
	TCHAR WeightOnPlanet[255];
}stringcompare;
...
struct Chemistry
{
	double q1, q2, efficiency, workdone, CoPr, CoPac;
}ch;
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	hwnd = CreateWindow(szAppName,
		TEXT("WM_GESTURE Win32SDK 2018 Project"),
		WS_OVERLAPPEDWINDOW,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
		CW_USEDEFAULT,
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		DrawText(hdc, "Press Space Bar To Continue...!!", -1, &rc, DT_CENTER | DT_SINGLELINE | DT_RIGHT);
		...
		SetBkMode(hdc, TRANSPARENT);
		SetTextColor(hdc, RGB(255, 248, 220));
		TextOut(hdc, 1030, 660, TEXT("Group: WM_GESTURE"), 23);
		TextOut(hdc, 1080, 620, TEXT("AstroMediComp"), 15);
		TextOut(hdc, 10, 660, TEXT("Batch : Win32SDK - 2k18"), 23);
		TextOut(hdc, 70, 620, TEXT("Atharva A. Pore"), 15);
...
BOOL CALLBACK DlgProc(HWND hDlg, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	static double mass, weight;
	sprintf_s(stringcompare.WeightOfPerson, "%f", ph.weight);
	sprintf_s(stringcompare.MassOfPerson, "%f", mass);
	sprintf_s(stringcompare.GravityOfPlanet, "%f", ph.gravity);
	sprintf_s(stringcompare.WeightOnPlanet, "%f", weight);
	...
	case WM_COMMAND:
		...
		case EN_SETFOCUS:
			...
			if (HandleIs == GetDlgItem(hDlg, ID_ETEARTHWGHT))
			{
				GetDlgItemText(hDlg, ID_ETMASS, mas, 7);
				ph.ms = atof(mas);
				if (ph.ms != '\0')
				{
					ph.PersonW = PersonWeight(ph.ms);
					sprintf_s(str, "%f", ph.PersonW);
					SetWindowText(GetDlgItem(hDlg, ID_ETEARTHWGHT), str);
...
//MatheMatics Implementation
		...
		case ID_PBTRIANGLES:
			if (ID_RBSIDE)
			{
				...
				if ((mm.sx1 == '\0' && mm.sx2 == '\0' && mm.sx3 == '\0') && (mm.sy1 == '\0' && mm.sy2 == '\0' && mm.sy3 == '\0'))
				{
		...
		case ID_PBTRIANGLEA:
			...
			mm.angle3 = atof(a3);
			if ((mm.angle1 == NULL || mm.angle2 == NULL || mm.angle3 == NULL))
			{
				MessageBox(hDlg, TEXT("Angles Are Missing...!!! OR All Fields Are 0"), TEXT("ERROR"), MB_OK | MB_ICONERROR);
			}
			else
			{
...
//			Physics Data
			if(IsDlgButtonChecked(hDlg, ID_RBPHYSICS))
			{
					...
					fprintf_s(file, "Weight Of a Person On Earth		:	%f\n", ph.weight);
					fprintf_s(file, "Mass Is					:	%f\n", mass);
					fprintf_s(file, "Planet's Gravity			:	%f\n", ph.gravity);
					fprintf_s(file, "Weight Of a Person On Selected Planet	:	%f\n", weight);
					MessageBox(hwnd, TEXT("Data is saved successfully...!!"), TEXT("DataEntry Confirmation"), MB_OK | MB_ICONINFORMATION);
					fclose(file);

			}
//			Mathematics Data
			else if (IsDlgButtonChecked(hDlg, ID_RBMATHS))
			{
					//Triangle Data
...
//			Chemistry Data
			else if (IsDlgButtonChecked(hDlg, ID_RBCHEMISTRY))
			{
			...
			{
				MessageBox(hDlg, TEXT("Please Choose The Subject\n Because There Is No Data To Save...!! "), TEXT("ERROR"), MB_OK | MB_ICONERROR);
			}
